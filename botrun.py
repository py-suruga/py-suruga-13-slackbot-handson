...
def generate_shizutenki_button(city_code_maps: list) -> str:
    """
...
@slack_events_adapter.on("message")
def tenki_button_question(event_data):
    """
    # Livedoor 天気予報を聞く Intaractiveバージョン
    # shizutenki
    # 今日の天気は ** 気温は**℃です！
    """

    # botが反応する正規表現パターン
    message_pattern = "shizutenki"
    ...
    if message.get("subtype") is None and message.get("bot_id") is None:
        ...
        if matchobj:
            # メッセージを返す
            channel = message["channel"]
            question_button_block = generate_shizutenki_button(LD_TENIKI_CITY_CODE_MAPS)
            slack_client.chat_postMessage(channel=channel, blocks=question_button_block)
